...
export const ServiceCard = ({ 
  ...
  return (
    <FadeIn delay={delay}>
      ...
      >
        <div>
          {/* Visual Thumbnail Header (Only if not hidden) */}
          {!hideVisualImage && visualImage && (
          ...
          <div className="p-6">
            <div className="flex justify-between items-center mb-4">
              <Link to={`/services/${slug}`} className="w-12 h-12 bg-brand-primary/10 border border-brand-primary/20 rounded-2xl flex items-center justify-center text-brand-primary group-hover:bg-brand-primary group-hover:text-white transition-all duration-300 shadow-xs">
                <Icon size={24} />
              </Link>
              {hideVisualImage && visualBadge && (
                <span className="bg-slate-100 text-slate-700 text-[10px] font-bold tracking-wider uppercase px-2.5 py-1 rounded-full border border-slate-200/80">
                  {visualBadge}
                </span>
              )}
            </div>
        ...
        <div className="p-6 pt-3 border-t border-slate-100 mt-auto flex flex-col sm:flex-row items-center gap-2">
          ...
          <Link 
            to="/#contact" 
            className="w-full sm:w-1/2 py-2 px-2.5 text-center text-xs font-bold rounded-xl bg-brand-primary text-white hover:bg-brand-hover shadow-xs transition-all duration-200 flex items-center justify-center whitespace-nowrap"
          >
            Get Consultation
          </Link>
        </div>
