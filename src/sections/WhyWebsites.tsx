        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center animate-fadeIn">
           ...
           <FadeIn delay={0.2} className="relative">
              ...
              <div className="relative bg-brand-bg rounded-[3rem] p-6 sm:p-10 border border-brand-border shadow-2xl">
                 ...
                 <div className="flex bg-white/80 backdrop-blur-sm p-1.5 rounded-2xl border border-brand-border/40 gap-1.5 mb-8 overflow-x-auto">
                    {[
                      { id: 'speed', label: '💻 Web & CMS Design', color: 'text-emerald-500' },
                      { id: 'leads', label: '📱 Social Marketing', color: 'text-brand-primary' },
                      { id: 'growth', label: '📈 Traffic Analytics', color: 'text-indigo-500' }
                    ].map(tab => (
                      <button
                        key={tab.id}
                        onClick={() => setActiveSegment(tab.id as any)}
                        className={`flex-1 min-w-[130px] py-3 text-xs sm:text-sm font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-1 cursor-pointer whitespace-nowrap ${
